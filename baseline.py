...
def preprocessData(dataFilePath, mode):
    ...
    with io.open(dataFilePath, encoding="utf8") as finput:
        ...
        for line in finput:
            ...
            preConv = emoji.demojize((line[1].lower()))
            preConv = re.sub('_',' ', preConv )
            preConv = re.sub(':','', preConv )
            u1.append(preConv)
            
            preConv = emoji.demojize((line[2].lower()))
            preConv = re.sub('_',' ', preConv )
            preConv = re.sub(':','', preConv )
            u2.append(preConv)
            
            preConv = emoji.demojize((line[3].lower()))
            preConv = re.sub('_',' ', preConv )
            preConv = re.sub(':','', preConv )
            u3.append(preConv)
            ...
            preConv = emoji.demojize((conv.lower()))
            preConv = re.sub('_',' ', preConv )
            preConv = re.sub(':','', preConv )
            conversations.append(preConv)
            print(preConv)
